...
When running `verdi code show vasp-std@perlmutter`, we should see the specifics (just as in [AiiDA-VASP tutorial](https://aiida-vasp-plugin.readthedocs.io/en/latest/getting_started/configuration.html))

## Adding POTCARs

Since I work with `pymatgen`, we will upload the potentials from such source. You need to have `pymatgen` in your environment installed and the `POTCAR` files configured appropriately (see the [`POTCAR` Setup](https://pymatgen.org/installation.html#potcar-setup) section).
